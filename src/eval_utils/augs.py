...
from eval_utils.augmentations import *
...
def get_frame_transform(frame_transform_str, img_size):
    ...
    if 'all' in fts:
        tt = [torchvision.transforms.RandomResizedCrop(
            img_size, scale=(0.6, 1), interpolation=Image.BICUBIC),
...
def get_train_transforms(args):
    ...
    transform = transforms.Compose([
        RandomSizedCrop(size=args.img_size, consistent=False, p=1.0),
        RandomHorizontalFlip(consistent=False),
        ColorJitter(brightness=0.4, contrast=0.4, saturation=0.2, hue=0.1, p=1.0),
