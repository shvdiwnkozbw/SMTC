...
def dump_predictions(pred, lbl_set, img, prefix):
    ...
    # pred_dist = cv2.resize(pred, (int(res[0].item()), int(res[1].item())))[:]
...
def mem_efficient_batched_affinity(query, keys, model, mask, temperature, topk, long_mem, device):
    '''
    ...
    for b in range(0, keys.shape[2], bsize):
        ...
        for pb in range(0, _k.shape[-1], pbsize):
            # A = calculate_attention(_q[..., pb:pb+pbsize], _k, _cls_q, _cls_k, model)
            A = torch.einsum('ijklm,ijkn->iklmn', _k, _q[..., pb:pb+pbsize]) 
            # A = torch.einsum('ijklm,ijkn->iklmn', torch.log(_k), _q[..., pb:pb+pbsize]) 
