...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 1000, 'input_size': (3, 192, 384), 'pool_size': None,
...
default_cfgs = {
    ...
    'swinv2_up_tiny_window16_256': _cfg(),
}
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=[192, 384], patch_size=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        super().__init__()
...
class SwinTransformerBlock(nn.Module):
    ...
    def _attn(self, x):
        #print("x.shape "+ str(x.shape))
        #print("self.input_resolution "+ str(self.input_resolution))
        H, W = self.input_resolution
        B, L, C = x.shape
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(
            self, dim, input_resolution, depth, num_heads, window_size,
            mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0., drop_path=0.,
            norm_layer=nn.LayerNorm, upsample=None, pretrained_window_size=0):
        ...
        #print("BasicLayer_up self.dim" + str(dim))
        #print("BasicLayer_up self.input_resolution" + str(input_resolution))
        # build blocks
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                dim=dim, input_resolution=input_resolution,
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(
            self, img_size=[192,384], patch_size=4, in_chans=3, num_classes=1000, global_pool='avg',
    ...
    def forward_head(self, x):
        x = einops.rearrange(x, 'b (h w) c -> b c h w', h = self.patch_embed.grid_size[0] // (2 ** (self.num_layers-1)))
        return x
...
class SwinTransformerV2_up(nn.Module):
    r""" Swin Transformer V2
        A PyTorch impl of : `Swin Transformer V2: Scaling Up Capacity and Resolution`
            - https://arxiv.org/abs/2111.09883
    Args:
    ...
    def __init__(
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer_up(
                ...
                input_resolution=(
                    self.img_size[0] // (2 ** (self.num_layers+1-i_layer)),
                    self.img_size[1] // (2 ** (self.num_layers+1-i_layer))),
                depth=depths[i_layer],
                num_heads=num_heads[i_layer],
                window_size=window_size,
                mlp_ratio=mlp_ratio,
        ...
        self.norm = norm_layer(int(self.num_features / (self.num_layers+1)))
        #self.head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
        self.up = FinalPatchExpand_X4(input_resolution=(self.img_size[0]//patch_size,self.img_size[1]//patch_size),dim_scale=4,dim=embed_dim)
    ...
    def forward_features(self, x):
        #print("1 forward_features x.shape" + str(x.shape))
        #print("self.img_size[0] // (2 ** (self.num_layers+1)) " + str(self.img_size[0] // (2 ** (self.num_layers+1))))
        x = einops.rearrange(x, 'b h w c-> b (h w) c ', h = self.img_size[0] // (2 ** (self.num_layers+1)))
        #print("2 forward_features x.shape" + str(x.shape))
        for layer in self.layers:
            x = layer(x)
            #print("Up layer x.shape " + str(x.shape))
        x = self.norm(x)  # B L C
        return x

    def up_x4(self, x):
        x = self.up(x)
        x = einops.rearrange(x, 'b (h w) c -> b c h w', h = self.img_size[0])
        return x
...
def _create_swin_transformer_v2_up(variant, pretrained=False, **kwargs):
    model = build_model_with_cfg(
        SwinTransformerV2_up, variant, pretrained,
        pretrained_filter_fn=checkpoint_filter_fn,
        **kwargs)
    return model
